...
- On this workstation, the MCP-owned credentials file is
  `/Users/tsardet/.config/mcp-legifrance/.env`. For local live calls, set
  `LEGIFRANCE_ENV_FILE` to that absolute path; never print or copy its contents.
...
## Search tools and query syntax

Use the public, corpus-specific tools: `Search_Cour_Cassation`,
`Search_Cour_Appel`, `Search_Conseil_Etat`, `Search_CAA`,
`Search_Premiere_Instance`, `Search_Code`, and
`Build_Research_Corpus`. The legacy generic alias `recherche_jurisprudence`
has been removed; do not restore or call it.

All public search queries share these rules:

- quotation marks delimit an exact expression: `"faute grave"`;
- `ET` binds more tightly than `OU`: `A OU B ET C` means `A OU (B ET C)`;
- parentheses override grouping: `(A OU B) ET C`;
- without an explicit operator, unquoted words are joined by `ET`;
- article references such as `L. 1235-3` are normalized automatically.

`Search_Premiere_Instance` is the sole default-operator exception: without an
explicit `ET` or `OU`, it joins unquoted words with `OU` to broaden its limited
corpus. Write `ET` explicitly when every term is required.

Keep this contract centralized in `config/mcp_definitions.py` and covered by
cross-tool tests. Examples must distinguish literal quotation marks from
typographic punctuation so an exact expression cannot be mistaken for a list
of ordinary keywords.
